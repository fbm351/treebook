...
class UserFriendshipTest < ActiveSupport::TestCase
    ...
    test "that creating a friendship works without raising an exception" do
        assert_nothing_raised do
            UserFriendship.create user: users(:fred), friend: users(:mike)
        end
    end

    test "that creating a friendship based on user id and friend id works" do
        UserFriendship.create user_id: users(:fred).id, friend_id: users(:mike).id
        assert users(:fred).pending_friends.include?(users(:mike))
    end

    context "a new instance" do
        setup do
            @user_friendship = UserFriendship.new user: users(:fred), friend: users(:mike)
        end
    ...
    context "#send_request_email" do
        setup do
            @user_friendship = UserFriendship.create user: users(:fred), friend: users(:mike)
        end
    ...
    context "#mutual_friendship" do
        setup do
            UserFriendship.request users(:fred), users(:bob)
            @friendship1 = users(:fred).user_friendships.where(friend_id: users(:bob).id).first
            @friendship2 = users(:bob).user_friendships.where(friend_id: users(:fred).id).first
        end
    ...
    context "#accept!" do
        setup do
            @user_friendship = UserFriendship.request users(:fred), users(:mike)
        end
    ...
    context ".request" do
        should "create two user friendships" do
            assert_difference 'UserFriendship.count', 2 do
                UserFriendship.request(users(:fred), users(:mike))
            end
        end
    ...
    context "#block!" do
        ...
        should "not allow new request once blocked" do
            @user_friendship.block!
            uf = UserFriendship.request users(:fred), users(:mike)
            assert !uf.save
        end
    end
end
